...
open class SurveyViewController: UIViewController {
    ...
    @objc public func tableViewTapped(sender: UITapGestureRecognizer) {
        if (self.cellDataDelegate!.getComplete()) {
            self.performSegue(withIdentifier: "finishQuestions", sender: self.cellDataDelegate!.getAnswers())
        }else{
    ...
    override open func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "finishQuestions" {
            if let nextVC = segue.destination as? SummaryViewController {
                nextVC.results = sender as! [String:Any]
            }
        }
    }
